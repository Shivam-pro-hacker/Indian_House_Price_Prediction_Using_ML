...
COLUMN_MAP = {
    ...
    "parking": "parking_space",   # IMPORTANT FIX
    "security": "security",
...
for new_col, old_col in COLUMN_MAP.items():
    if old_col not in df.columns:
        raise ValueError(f"❌ Required column missing in dataset: {old_col}")
...
def normalize_yes_no(val):
    return "yes" if str(val).lower() in ["yes", "y", "1", "true"] else "no"

df["parking"] = df["parking"].apply(normalize_yes_no)
df["security"] = df["security"].apply(normalize_yes_no)

# =====================================================
# REDUCE RARE CATEGORIES (VERY IMPORTANT)
# =====================================================
...
# =====================================================
# LIMIT DATA SIZE (PREVENT FREEZE)
# =====================================================
if len(df) > 50000:
    df = df.sample(n=50000, random_state=42)
    print("⚠ Dataset reduced to 50,000 rows for faster training")
...
model = RandomForestRegressor(
    ...
    min_samples_leaf=5,
    random_state=42,
    n_jobs=1   # IMPORTANT: prevents Windows freeze
)
...
joblib.dump(X.columns.tolist(), os.path.join(MODEL_DIR, "rf_columns.pkl"))
